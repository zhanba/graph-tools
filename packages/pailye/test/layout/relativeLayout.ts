...
function applyHorizontalRules(relativeConstraints, childPositions, childName, inlineSize) {
  ...
  const c = relativeConstraints[childName] || {};
  if (c.leftOf && childPositions[c.leftOf]) position.right = childPositions[c.leftOf].left;

  if (c.rightOf && childPositions[c.rightOf]) position.left = childPositions[c.rightOf].right;

  if (c.alignLeft && childPositions[c.alignLeft]) position.left = childPositions[c.alignLeft].left;
...
function applyVerticalRules(relativeConstraints, childPositions, childName, blockSize) {
  ...
  const c = relativeConstraints[childName] || {};
  if (c.above && childPositions[c.above]) position.bottom = childPositions[c.above].top;

  if (c.below && childPositions[c.below]) position.top = childPositions[c.below].bottom;

  if (c.alignTop && childPositions[c.alignTop]) position.top = childPositions[c.alignTop].top;
...
export class RelativeLayout implements AbstractLayout {
  ...
  async layout(children, edges, constraints, styleMap) {
    ...
    for (const childName of sortedChildNamesHorizontal) {
    ...
    for (const childName of sortedChildNamesVertical) {
      applyVerticalRules(
        relativeConstraints,
        childPositions,
        childName,
      ...
      const fragment = await measureChild(childrenMap[childName], childPositions[childName]);
